#!/usr/bin/env python3
"""
Update On Review - Fix Critical Quiz Questions (PROD ONLY)
Reads critical question IDs → Fetches from Supabase PROD → Analyzes with Gemini → Generates SQL fixes
Replicates exact logic from updateonreview.py but for quiz questions
"""
...
class UpdateOnReviewQuiz:
    def __init__(self):
        ...
        self.sql_file = 'sql_quiz_updates_from_review.sql'
    ...
    def _init_sql_file(self):
        ...
        with open(self.sql_file, 'w') as f:
            f.write("-- Update Critical Quiz Questions (PROD)\n")
            f.write("-- Generated from updateonreview_quiz.py\n")
    ...
    def load_critical_question_ids(self):
        ...
        critical_ids_file = Path(__file__).parent / "quiz_critical_question_ids.txt"
        ...
        try:
            ...
            # Extract IDs (format: q_XX_m_XX_Y_Z)
    ...
    def get_questions(self, question_ids, limit=None):
        ...
        try:
            ...
            for q_id in ids_to_fetch:
                # Query the 'question' table by id field
                response = self.supabase.table('question').select('*').eq('id', q_id).execute()
    ...
    def analyze_with_gemini(self, question):
        ...
        try:
            # Parse options - might be JSON string or array
            ...
            # Get current correct answer - parse if it's PostgreSQL format '{0}' or '{1,2}'
            current_answer = question.get('correct_answer', [])
            if isinstance(current_answer, str):
                # Parse PostgreSQL format: '{0}' or '{1,2}'
                try:
                    current_answer = [int(x.strip()) for x in current_answer.strip('{}').split(',') if x.strip()]
                except:
                    current_answer = []

            prompt = f"""You are a subject matter expert in AWS certifications and cloud concepts. Evaluate this quiz question strictly.

QUESTION: {question.get('text', '')}
OPTIONS:
{options_formatted}
CURRENT CORRECT ANSWER INDICES: {current_answer}

Task: Verify whether the marked correct answer index/indices are factually accurate based on the question content and AWS best practices.
...
{{
    ...
    "reasoning": "brief explanation of why these indices are correct"
            ...
            return {"answer_is_correct": True, "correct_answer_indices": current_answer}
        except Exception as e:
    ...
    def generate_explanation(self, question, correct_answer_indices):
        ...
        try:
            ...
            prompt = f"""You are a subject matter expert in AWS certifications. Write a detailed explanation for this quiz question.
...
Return only the explanation text, no JSON, no markdown."""
    ...
    def run(self, limit=None):
        ...
        print("="*80)
        print("   🔍 UPDATE ON REVIEW - QUIZ QUESTIONS (PROD ONLY)")
        print("="*80 + "\n")
    ...
    def _finalize_sql_file(self):
        ...
        with open(self.sql_file, 'a') as f:
            f.write("COMMIT;\n")
        print(f"✅ sql_quiz_updates_from_review.sql finalized ({self.total_sql_statements} updates)")
    
    def _write_json_report(self):
        """Write JSON report"""
        with open('analysis_report_quiz.json', 'w') as f:
            json.dump({
                'total_analyzed': len(self.analysis_results),
                'issues_found': self.total_sql_statements,
                'analyses': self.analysis_results
            }, f, indent=2)
        print("✅ analysis_report_quiz.json")
...
if __name__ == "__main__":
    ...
    try:
        UpdateOnReviewQuiz().run(limit=limit)
    except Exception as e:
        print(f"❌ Error: {e}")
