...
INSERT INTO public.question (
    ...
    'Computer Vision analyzes images and videos, NLP processes human language, and Inference makes predictions using trained models.',
    NOW(),
    'quiz_domain5_1',
    NOW(),
    10,
    '{"left": ["Computer Vision", "NLP", "Inference"], "right": ["Image analysis", "Language processing", "Model predictions"]}'::json,
    '{"left": [0, 1, 2], "right": [0, 1, 2]}'::json,  -- left[0] matches right[0], left[1] matches right[1], left[2] matches right[2]
    'm_5_1'
);
...
-- pairs.right = ["Image analysis", "Language processing", "Model predictions"] (indices: 0, 1, 2)

-- matches.left  = [0, 1, 2]  (all left items in order)
-- matches.right = [0, 1, 2]  (corresponding right item indices)

-- This means:
-- - Computer Vision (left[0]) matches with "Image analysis" (right[0])
-- - NLP (left[1]) matches with "Language processing" (right[1]) 
-- - Inference (left[2]) matches with "Model predictions" (right[2])
...
-- When the API generates a matching question, it creates this structure:
{
  ...
  "pairs": {
    "left": ["Deep Learning", "Computer Vision", "NLP"],
    "right": ["Neural networks", "Image recognition", "Text analysis"]
  },
  "matches": {
    "left": [0, 1, 2],
    "right": [0, 1, 2]
  ...
  "correct_answer": null,
  "explanation": "Deep Learning uses neural networks, Computer Vision handles image recognition, and NLP focuses on text analysis."
}
