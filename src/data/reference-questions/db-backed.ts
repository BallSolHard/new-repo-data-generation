...
export async function selectFewShotExamplesV2(
  ...
  return await selectModeMatchingInCode(
    certCode,
    domainId,
    params.genMode,
    params.questionType,
 ...
 */
async function selectModeMatchingInCode(
  certCode: string,
  domainId: string,
  genMode?: GenMode,
  questionType?: QuestionType,
  ...
  let candidates = await getReferencesForDomain(certCode, domainId, questionType);
  ...
  if (genMode) {
    ...
    return await selectFewShotExamples(certCode, domainId, questionType, undefined, count);
  }
